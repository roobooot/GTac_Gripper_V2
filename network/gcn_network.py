...
class GCN_test(torch.nn.Module):
    ...
    def __init__(self, numFeatures, numClasses): #, Features_dimension

        super().__init__()

        #self.flatten1 = GCNConv(Features_dimension, 1)
        self.conv1 = GCNConv(numFeatures, 16)
        # self.conv2 = GCNConv(16, 32)
        # self.conv3 = GCNConv(32, 64)
        # self.conv4 = GCNConv(64, 16)
        # self.conv5 = GCNConv(16, 32)
        self.fc1 = torch.nn.Linear(768, 128)  #changed to our datasize
    ...
    def forward(self, data):
        ...
        data.x = F.log_softmax(data.x.view(1, 15), dim=1)  #change to the number of the classes
        log.debug(data.x.size())
        return data.x
...
class GCN_32_64(torch.nn.Module):
    ...
    def __init__(self, numFeatures, numClasses):
        ...
        self.conv1 = GCNConv(numFeatures, 16)
        self.conv2 = GCNConv(16, 32)
        self.fc1 = torch.nn.Linear(1536, 128)  # changed to our datasize
        self.fc2 = torch.nn.Linear(128, numClasses*1)
...
'''
model=GCN_test(numFeatures=204,numClasses=3)
x = torch.randn(204)#
y = model(x)
